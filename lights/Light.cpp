...
int Light::setLightBacklight(const LightState &state)
{
    ...
    return writeInt(BACKLIGHT_FILE, brightness);;
}
...
int Light::setSpeakerLightLocked(const LightState &state)
{
    ...
    writeInt(SNS_LED_FILE, 0);

    if (blink) {
        writeInt(RED_LED_FILE, 1);
        writeInt(GREEN_LED_FILE, 1);
        writeInt(BLUE_LED_FILE, 1);
        writeInt(SNS_LED_FILE, 1);
    } else {
        writeInt(RED_LED_FILE, red);
        writeInt(GREEN_LED_FILE, green);
        writeInt(BLUE_LED_FILE, blue);
	writeInt(SNS_LED_FILE, colorRGB);
    }
